...
public class Lab2 {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("=== Integer Matrix Processing Program (Lab 2) ===");

        String rowPrompt = String.format("Enter the number of rows (%d–%d): ",
                MIN_ROWS_COLS, MAX_ROWS_COLS);
        int rows = getMatrixDimension(scanner, rowPrompt);

        String colPrompt = String.format("Enter the number of columns (%d–%d): ",
                MIN_ROWS_COLS, MAX_ROWS_COLS);
        ...
        System.out.printf("Arithmetic Mean: %.2f%n", avg);
    ...
    private static int getMatrixDimension(Scanner scanner, String promptMessage) {
        ...
        String rangeError = String.format(
                "Error: Enter an integer from %d to %d.",
                MIN_ROWS_COLS, MAX_ROWS_COLS
        );
    ...
    private static int[][] createMatrixManual(Scanner scanner, int rows, int cols) {
        ...
        System.out.printf("Enter matrix elements (%d×%d):%n", rows, cols);
    ...
    private static int[][] createMatrixRandom(int rows, int cols) {
        ...
        System.out.printf(
                "Matrix generated with random integers between %d and %d.%n",
    ...
    private static double calculateGeometricMean(int[][] matrix) {
        ...
        if (positiveCount == 0) {
            System.out.println(
                    "No positive elements found. Cannot calculate Geometric Mean."
            );
            return 0.0;
        }
